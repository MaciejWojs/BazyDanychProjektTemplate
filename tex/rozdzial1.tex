...
% * SPOSOBY UŻYWANIA MAKRA HERE * # 

% ? Listingi
% Parametr #1: Opis listingu (wyświetla się bezpośrednio pod listingiem)
% Parametr #2: Nazwa pliku oraz ID do oznaczania (ważne, żeby był w katalogu kod oraz jego rozszerzenie to txt) 
% Parametr #3: Ścieżka do pliku z kodem

\ListingFile{OpisPromptu}{prompt}{kod/prompt.txt}
...
% ? Zdjęcia
% OPCJONALNY Parametr #1: Szerokość zdjęcia (domyślnie jest to szerokość paragrafu ale jak się poda w [#1] to wtedy zmienia się na podaną wartość)
% Parametr #2: Nazwa pliku z rozszerzeniem (podajemy katalog w którym jest plik i jego rozszerzenie np. rys/nazwa-rysunku.png)
% Parametr #3: Opis tego co jest na rysunku (wyświetla się bezpośrednio pod rysunkiem) 
